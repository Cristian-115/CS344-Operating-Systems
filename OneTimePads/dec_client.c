...
//prints error if we couldnt write to file
void writeerror(int charwritten) {
    if (charwritten < 0) {
        error("CLIENT: ERROR writing to socket");
    }
}
//prints error if we couldnt read from file 
...
int main(int argc, char* argv[])
{
    ...
    long int messageLength; // for use with findsize
    long int keyLength;
    ...
    if (buffer[0] != 'd')                                                          // checks to see if we recevied hello client from the server
