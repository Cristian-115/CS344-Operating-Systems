...
* it was needed in orger to compare lengths of key file and plaintext files
*/
...
char* encrypt(char* plaintext, char* key) { //encrypt a given message
    int i;
    char n;
    char c;
    ...
    {
        //if we see a newline then its the EOF
        if (plaintext[i] == '\n') {
            break;
        }
        //error handling for bad characters 
        ...
        if (plaintext[i] == ' ') {
            n = 26; //last possible val so just return 26
        }
        else {

            n = plaintext[i] - 65; // subtract A from our current character

        }
        if (key[i] == ' ') {
            c = 26; //last possible val so just return 26
        }

        else {

            c = key[i] - 65;  // subtract A from our current character

        }
        n = (n + c) % 27;// add out plaintext char and key char then mod by 27 to get our encrypted character
...
int main(int argc, char const* argv[])
{
    ...
    while (1)
    {
        ...
        case 0: {
            ...
            char* memo = "hello client";                                               //let server know we got the plaintext
            ...
            charsRead = send(establishedConnectionFD, encrypt(temp, key), strlen(temp), 0); // Send encrypted message back
