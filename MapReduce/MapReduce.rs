...
fn main() {
    ...
    let thread_2 = thread::spawn(move || map_data(&xs2[1]));
    ...
    //creates a vector to hold intermediate sums
    let mut inter_sums_two : Vec<usize> = Vec::new();
    for i in 0..num_partitions {

        //create a clone of the partition data 
        let clone = z.clone();

        // 3. Creates one thread per partition and uses each thread to concurrently process one partition
        let new_thread = thread::spawn(move || map_data(&clone[i]));
        let _r3 = new_thread.join().unwrap();

         // 4. Collects the intermediate sums from all the threads
        inter_sums_two.push(_r3);
    }
      // 4. Collects the intermediate sums from all the threads
    
    println!("Intermediate sums = {:?}",  inter_sums_two);
       // 5. Calls reduce_data to process the intermediate sums
      let sums = reduce_data(& inter_sums_two);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let size = v.len() / num_partitions;// this determines the max size of the partition
    let mut vec: Vec<Vec<usize>> = Vec::new(); 
    let mut remainder = v.len() % num_partitions;// holds result after modding v.len by num partitions
    let mut start = 0;
    let mut end = size; //we will stop at the max  partition size 

    
    for i in 0..num_partitions {

        // push each val to into are new vector
        let mut x : Vec<usize> = Vec::new();
        for j in start..end{
            x.push(v[j]);
        }
        vec.push(x); // fuch the entire x vector into vec

        //when the remainder is 0 our partitions are even 
        if remainder != 0 {
            vec[i].push(v[end]);
            start += size + 1;
            end += size + 1;
            remainder -= 1; //decrement our remainder 
        } else {
